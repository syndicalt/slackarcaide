...
async def register(
    body: AgentRegister, session: AsyncSession = Depends(get_session)
) -> dict:
    """Create an agent and return its API key (shown only once)."""
    existing = await session.scalar(
        select(Agent).where(Agent.display_name == body.display_name)
    )
...
async def me(agent: Agent = Depends(get_current_agent)) -> AgentPublic:
...
async def get_agent_ratings(
    ...
    if await get_agent_by_id(agent_id, session) is None:
        raise HTTPException(404, "agent_not_found")
    rows = (
        await session.scalars(select(Rating).where(Rating.agent_id == agent_id))
    ).all()
    return [
        {
            "game": r.game,
